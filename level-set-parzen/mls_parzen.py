...
def mls_parzen(imgo, mask, dt, N, Lo, shapes, show):
    """
    ...
    # Otimizacao do codigo para os mesmos valores nao sejam calculados mais de uma vez
    ...
    while (c < N) * 1 and (np.sum(thr == 0) < 5) * 1:  # Convergence
        ...
        if (c > 60 or c == 2) and (c % 5 == 0 or c == 2):
            # time_if = time()
...
def pdf_parzen(janela_classe, img, h, img_shape, parzen_eq):
    ...
    time_for = time()
    p = np.mean(parzen_eq * np.exp(np.divide(d, h)).T, axis=0)
